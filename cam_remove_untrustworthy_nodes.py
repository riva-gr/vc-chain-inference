#!/usr/bin/env python3
"""
CAM trust-aware variant: NODE-LEVEL filtering (Sec. IV-B).
Requires cam.py in the same directory.
"""
...
from cam import (acquisition_counts, assemble_chains, corresponding_gt,
                 load_rows, remove_subchains)

NODE_TRUST_THRESHOLD = 0.5
LINK_TRUST_THRESHOLD = 0.5
NOISE_TRUST_HIGH = 0.49
TRUST_SEED = 4
VARIANT_DIR = "CAM_remove_untrustworthy_nodes"


def assign_trust(creds: list[str], gt_creds: set[str],
                 noise_high: float = NOISE_TRUST_HIGH,
                 seed: int = TRUST_SEED) -> dict[str, float]:
    """Issuer-trust value per credential, over the sorted credential list."""
    random.seed(seed)
    return {c: round(random.uniform(0.50, 1.0), 2) if c in gt_creds
            else round(random.uniform(0.0, noise_high), 2) for c in creds}


def trusted_chain_links(counts: np.ndarray, creds: list[str],
                        min_support: float, risup: float, n_rows: int,
                        trust: dict[str, float]) -> list[list[str]]:
    """Eq. (1) on pairs, restricted to trustworthy endpoints."""
    support = counts + counts.T
    ratio = np.divide(counts, support,
                      out=np.zeros(counts.shape, dtype=float),
                      where=support > 0)
    keep = (support > 0) & (support >= n_rows * min_support) & (ratio >= risup)
    trusted = np.array([trust[c] >= LINK_TRUST_THRESHOLD for c in creds])
    keep &= trusted[:, None] & trusted[None, :]
...
def main() -> None:
    ...
    ap.add_argument("risup", nargs="?", type=float, default=1.0,
                    help="relative in-order support threshold")
    ...
    ap.add_argument("--results-dir", type=Path,
                    default=Path("Results_modified/CAM"))
    ...
    # Node-level filtering: entities whose average credential trust falls
    # below the threshold are excluded from the dataset.
    filtered_rows = []
    for row in rows:
        values = [trust[c] for c in row if c in trust]
        if values and sum(values) / len(values) >= NODE_TRUST_THRESHOLD:
            filtered_rows.append(row)

    counts = acquisition_counts(filtered_rows, creds)
    links = trusted_chain_links(counts, creds, args.min_support, args.risup,
                                len(rows), trust)
    ...
    if args.verbose:
        print(f"records kept: {len(filtered_rows)}/{len(rows)} "
              f"(removed {len(rows) - len(filtered_rows)})  "
