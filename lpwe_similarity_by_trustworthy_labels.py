#!/usr/bin/env python3
"""
LPwE trust-aware variant: LABEL-LEVEL filtering (Sec. IV-C).
...
TRUST_SEED = 4
VARIANT_DIR = "LPwE_similarity_by_trustworthiness"
...
def main() -> None:
    ...
    # Label-level filtering: similarity from TRUSTED label subsets only.
    trusted_labels = np.array([trust[l] >= LABEL_TRUST_THRESHOLD
                               for l in labels])
    B_all = membership_matrix(rows, labels)
    A_all = jaccard_adjacency(B_all & trusted_labels[None, :],
                              args.similarity_threshold)

    keep = A_all.any(axis=1)                 # isolated nodes removed
    A = A_all[np.ix_(keep, keep)]
    B0 = B_all[keep]                         # propagation uses FULL labels
    ...
    if args.verbose:
        print(f"nodes kept: {B0.shape[0]}/{len(rows)}  "
              f"trusted labels: {int(trusted_labels.sum())}/{len(labels)}  "
              f"rounds (n): {n_rounds}")
