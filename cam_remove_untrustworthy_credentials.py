#!/usr/bin/env python3
"""
CAM trust-aware variant: CREDENTIAL-LEVEL filtering (Sec. IV-B).
...
from cam import (acquisition_counts, assemble_chains, chain_links,
                 corresponding_gt, load_rows, remove_subchains)

CRED_TRUST_THRESHOLD = 0.6
NOISE_TRUST_HIGH = 0.49
TRUST_SEED = 4
VARIANT_DIR = "CAM_remove_untrusworthy_credentials"
...
def main() -> None:
    ...
    # Credential-level filtering: untrustworthy credentials are dropped
    # from every record before the co-acquisition matrix is populated.
    filtered_rows = [[c for c in row if trust[c] >= CRED_TRUST_THRESHOLD]
                     for row in rows]
    ...
    if args.verbose:
        kept = sum(len(r) for r in filtered_rows)
        total = sum(len(r) for r in rows)
        print(f"records: {len(rows)}  credentials kept: {kept}/{total}  "
