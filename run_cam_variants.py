#!/usr/bin/env python3
"""
Runner: CAM trust-aware variants (Sec. IV-B).

Runs the three trust-aware variants of CAM -- credential-level,
node-level and link-level filtering -- over the ground-truth percentages
and experiment files, for every requested combination of minimum support
and relative in-order support. Each run prints its metrics; by default
they are also appended under Results_modified/CAM/<variant>/
...
SCRIPTS = ["cam_remove_untrustworthy_credentials.py",
...
def main() -> None:
    ...
    ap.add_argument("--min-supports", type=float, nargs="+", default=[0.01])
