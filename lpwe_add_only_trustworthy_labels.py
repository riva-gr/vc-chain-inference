#!/usr/bin/env python3
"""
LPwE trust-aware variant: PREDICTION-LEVEL filtering (Sec. IV-C).
...
TRUST_SEED = 4
VARIANT_DIR = "LPwE_add_only_trustworthy_labels"
...
def main() -> None:
    ...
    # Trust-agnostic graph and propagation (full label sets).
    ...
    # Prediction-level filtering: score AND trust must both pass.
    trusted_labels = np.array([trust[l] >= LABEL_TRUST_THRESHOLD
                               for l in labels])
    added = (scores > args.score_threshold) & trusted_labels[None, :]
