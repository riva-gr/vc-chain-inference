#!/usr/bin/env python3
"""
CAM trust-aware variant: LINK-LEVEL filtering (Sec. IV-B).
...
TRUST_SEED = 4
VARIANT_DIR = "CAM_trustworthy_links"
...
def main() -> None:
    ...
    if args.verbose:
        trusted = sum(1 for c in creds if trust[c] >= LINK_TRUST_THRESHOLD)
        print(f"records: {len(rows)}  trustworthy credentials: "
              f"{trusted}/{len(creds)}  links: {len(links)}  "
              f"chains: {len(chains)}")
        for chain in chains:
            print(chain)
