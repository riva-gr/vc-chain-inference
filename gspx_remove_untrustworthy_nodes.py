#!/usr/bin/env python3
"""
GSP-X trust-aware variant: NODE-LEVEL filtering (Sec. IV-A).
...
TRUST_SEED = 4
VARIANT_DIR = "GSP-X_remove_untrustworthy_nodes"
...
def main() -> None:
    ...
    # Node-level filtering: keep records whose average trust clears the bar.
    filtered_rows = []
    for row in rows:
    ...
    # Parity quirk: mining threshold anchored to the ORIGINAL N.
    frequent = mine_frequent_sequences(filtered_rows,
                                       args.min_support * len(rows))
    chains = remove_subchains(build_chains(frequent, filtered_rows,
                                           args.risup))
    ...
    if args.verbose:
        print(f"records kept: {len(filtered_rows)}/{len(rows)} "
              ...
              f"frequent: {len(frequent)}  chains: {len(chains)}")
        for chain in chains:
            print(chain)
